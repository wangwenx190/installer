...
4. 打开**Setup.iss**，根据需求，修改**MyAppID**、**MyAppName**和**MyAppVersion**等涉及到具体项目的信息
...
8. 若您想要控制安装程序只能安装新版本，而不能以旧版本覆盖新版本，请将代码中的`#define OnlyInstallNewVersion`取消注释，但由于版本号判断算法的缺陷，导致若您开启此功能，当主版本号与次版本号的值相等时，在已经安装过您的产品的机器上再次运行安装程序时，安装程序会报错，请您注意（此BUG我会尽量解决）
9. 若您有写INI条目、创建快捷方式或写入注册表条目的需求，请取消代码中**INI**、**Icons**或**Registry**区段的注释，并自行添加相关脚本
10. 在代码文件（**Setup.iss**）所在的目录下新建**App**文件夹
